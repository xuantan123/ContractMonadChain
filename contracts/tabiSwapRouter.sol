...
// File contracts/dex/DaiKoDexRouter.sol

// SPDX-License-Identifier: MIT

pragma solidity =0.6.6;

contract tabiSwapChainRouter is IUniswapRouter02 {
    using SafeMath for uint;
        ...
        returns (uint[] memory amounts)
    {
        ...
        _incrementSwapCount(msg.sender);
    ...
    function _incrementSwapCount(address user) internal {
        userSwapCount[user] += 1;
        
        // Emit event when user reaches mint eligibility (3 swaps)
        if (userSwapCount[user] == 3) {
            emit SwapMilestone(user, 3);
        }
    }
